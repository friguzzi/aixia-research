...
out.write('<table style="margin-left:auto; argin-right:auto;">\n<tr><td>Cluster</td><td>N. of documents</td><td>Top terms</td></tr>\n')
for i in range(true_k):
    print("Cluster %d;  %d;" % (i,card[i]), end='')
    out.write('<tr><td>'+str(i)+'</td><td>'+str(card[i])+'</td><td>')
    for ind in order_centroids[i, :9]:
        print(ind)
        print(' %s,' % terms[ind], end='')
        out.write(terms[ind]+', ')
    print(order_centroids[i,9])
    print(' %s,' % terms[order_centroids[i,9]], end='')
    out.write(terms[order_centroids[i,9]]+'</td></tr>\n')
    print()
out.write('</table></div>\n\n')
out.write('<div style="text-align:center">')
for i in range(true_k):
    ...
    print()
    out.write('</table>\n\n')
    out.write('<img src="cloud'+str(i)+'.png" width="500" />\n')
out.write('</div>\n\n')
out.close()
