...
def tosim(x):
    return math.exp(-math.pow(x,2)/2/0.25)

vec_tosim=np.vectorize(tosim)
    

out=open('members_norm.html','w')

people_class=joblib.load('classif-papers.pkl') 

print(len(people_class))
people_class.sort( key=lambda p: p['cognome']+' '+p['nome'])
out.write('<table>\n<tr><td>Member</td>')
for i in range(10):
    out.write('<td>'+str(i)+'</td>')
out.write('</tr>\n')
for i in range(len(people_class)):
    if not  people_class[i]['consent']=='sì':
     print(people_class[i]['nome']+" "+people_class[i]['cognome']+": "+people_class[i]['consent'])
    if people_class[i]['consent']=='sì':
        #print(people_class[i]['nome']+" "+people_class[i]['cognome']+": ")
        #print(people_class[i]['sim'])
        sim=vec_tosim(people_class[i]['sim'])
        s=np.mean(sim,axis=0)
        sd=np.std(sim,axis=0)
        print(s)
        sum=np.sum(s)
        out.write('<tr><td>'+people_class[i]['cognome']+" "+people_class[i]['nome']+'</td>')
        for j in range(10):
            out.write('<td>{:.3f}</td>'.format(s[j]/sum))
#            out.write('<td>{:.3f},{:.3f}</td>'.format(s[j],sd[j]))
        out.write('</tr>\n')
out.write('</table>\n\n')
out.close
