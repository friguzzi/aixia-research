...
print(len(socio))
for f in socio:
    print(socio[f]['cognome'])
    print(len(socio[f]['files']))
    texts=[]
    for file in socio[f]['files']:
        shakes = open(file, 'r')
        doc = shakes.read()
        text=doc
    ...
    fre_mat=vectorizer.transform(texts)
    c=km.transform(fre_mat)
    vec_tosim=np.vectorize(tosim)
    c1=c
    people.append({'nome':socio[f]['nome'],'cognome':socio[f]['cognome'],'consent':socio[f]['consent'],'sim':c1})


print(len(people))

#print(mat)
joblib.dump(people, 'classif-papers.pkl') 
